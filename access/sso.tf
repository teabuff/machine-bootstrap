...
# secret from the idp module's state (one-directional: access depends on idp, the
# client's wildcard callback does NOT depend on this IdP). identifier_path/scopes
# match the legacy bash. redirect_url is exported by the provider for reference.
